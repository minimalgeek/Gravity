...
public class RigidbodyFPS : MonoBehaviour
{
    ...
    private Vector3 CalculateVelocityChange(Vector3 inputVector)
    {
        ...
        if (inputVector.z > 0)
        {
            relativeVelocity.z *= (canRun && Input.GetKey(KeyCode.LeftShift)/*GetButton("Sprint")*/) ? runSpeed : walkSpeed;
        }
        else
        {
            relativeVelocity.z *= (canRun && Input.GetKey(KeyCode.LeftShift)/*GetButton("Sprint")*/) ? runBackwardSpeed : walkBackwardSpeed;
        }
